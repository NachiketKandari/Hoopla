...
from app.dependencies import require_user, get_current_user, is_admin_user, TEMPLATES_DIR
...
async def readme_page(request: Request, user: dict = Depends(require_user)):
    ...
    return templates.TemplateResponse(
        "pages/readme.html", {"request": request, "user": user, "content": content}
    )
...
async def dataset_page(request: Request, user: dict = Depends(require_user)):
    ...
    return templates.TemplateResponse(
        "pages/dataset.html",
        {"request": request, "user": user, "movies": movies, "movies_json": json.dumps(movies, indent=2)},
    )
